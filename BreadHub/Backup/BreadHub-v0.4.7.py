...
class BreadHub:
    ...
    def launch_script(self):
        ...
        try:
            ...
            self.root.destroy() # Close the manager after launching
    ...
    def add_new_script(self):
        ...
        browse_btn = ttk.Button(dialog, text="Browse", command=browse_path, style='Button.TButton')
        ...
        def confirm_add(event=none):
            ...
            if not name or not path:
                messagebox.showwarning("Incomplete", "Please provide both a script name and path")
                return
            if name in self.scripts:
                messagebox.showwarning("Exists", f"A script named '{name}' already exists.")
                return
            ...
            self.save_config()
            self.save_script_list()
            dialog.destroy()
            messagebox.showinfo("Added", f"Script '{name}' added successfully!")

        add_btn = ttk.Button(dialog, text="Add", command=confirm_add, style='Button.TButton')
        add_btn.pack(pady=(o, 10))
        ...
        ttk.Button(dialog, text="Add Script", command=add_script, style='Button.TButton').pack(pady=(5,10))
