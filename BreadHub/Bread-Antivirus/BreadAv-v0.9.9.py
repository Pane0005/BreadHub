...
class BreadDefender:
    def __init__(self, root):
        self.root = root
        self.root.title("BreadAv - Anti-Virus Protection")
        self.root.geometry("900x700")
        self.root.configure(bg=BG_COLOR)

        # Threat queue
        self.threat_queue = queue.Queue()
        self.quarantine_dir = os.path.expanduser("~/.breadav_quarantine")
        os.makedirs(self.quarantine_dir, exist_ok=True)

        # Security lists
        self.whitelist_file = os.path.expanduser("~/.breadav_whitelist.json")
        self.blacklist_file = os.path.expanduser("~/.breadav_blacklist.json")
    ...
    def create_widgets(self):
        # Title
        ttk.Label(self.root, text="Bread Antivirus", style='Title.TLabel').pack(pady=(10,0))
        ttk.Label(self.root, text="v 0.9.9 by Доктор Bread", style='SubTitle.TLabel').pack(pady=(0,10))
        ...
        license_text = ("BreadAv v0.9.8 by Доктор Bread\n"
                        "Copyright (c) 2026 Доктор Bread - BreadAv. All Rights Reserved.\n"
...
class SplashScreen:
    def __init__(self, root, duration=3):
        ...
        label = tk.Label(root, text="Bread AntiVirus", font=("FiraCode Nerd Font Mono", 24, "bold"), bg=BREAD_COLOR, fg=BG_COLOR)
